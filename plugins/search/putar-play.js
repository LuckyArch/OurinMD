...
const pluginConfig = {
    ...
    usage: ".putar-play <query>",
    example: ".putar-play komang",
    cooldown: 15,
    energi: 1,
    isEnabled: true
}
...
async function handler(m, { sock, text }) {
    ...
    if (!query) return m.reply(`🎵 *putar-play*\n\n> Contoh:\n\`${m.prefix}putar-play3 https://youtube.com/watch?v=Lmelm-s_PII\``)
    ...
    try {
        ...
        await execAsync(
        `ffmpeg -y -i "${inputFile}" -c:a libopus -b:a 64k "${outputFile}"`
        )
